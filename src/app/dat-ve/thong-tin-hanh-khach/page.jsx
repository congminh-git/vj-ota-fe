...
import PassengerInfomation from '@/components/thong-tin-hanh-khach/thong_tin_hanh_khach';
import Steps from '@/components/danh-sach-ve/steps';
import ContactInfomation from '@/components/thong-tin-hanh-khach/thong_tin_lien_he';
...
export default function PassengerInfomationPage() {
    ...
    const listBreadcrumb = [
        ...
        {
            title: 'Danh sách vé',
            uri: `/dat-ve/danh-sach-ve?cityPair=${cityPair}&departure=${departmentDate}&roundTrip=${roundTrip}&comeback=${returnDate}&currency=${currency}&adultCount=${adult}&childCount=${child}&infantCount=${infant}&departureCity=${departureCity}&arrivalCity=${arrivalCity}&activeSelectFlight=${'đi'}`,
        },
        { title: 'Thông tin hành khách', uri: '/dat-ve/thong-tin-hanh-khach' },
    ];
    ...
    const checkValidation = () => {
        ...
        listPassenger.listAdult.forEach((element, index) => {
            if (!element.lastName) {
                document.querySelector(`.thong-tin-hanh-khach-ho-${index}`).innerHTML = 'Vui lòng nhập họ';
                validateCount += 1;
            } else {
                document.querySelector(`.thong-tin-hanh-khach-ho-${index}`).innerHTML = '';
            }

            if (!element.firstName) {
                document.querySelector(`.thong-tin-hanh-khach-ten-dem-ten-${index}`).innerHTML =
            ...
            if (validateDob(element.dob)) {
                document.querySelector(`.thong-tin-hanh-khach-ngay-sinh-${index}`).innerHTML =
            ...
            if (!element.gender) {
                document.querySelector(`.thong-tin-hanh-khach-gioi-tinh-${index}`).style.borderBottom =
                    '1px solid rgb(239, 68, 68)';
                validateCount += 1;
            } else {
                document.querySelector(`.thong-tin-hanh-khach-gioi-tinh-${index}`).style.borderBottom = 'none';
            }

            if (index === 0) {
                if (!element.phoneNumber) {
                    document.querySelector(`.thong-tin-hanh-khach-sdt-${index}`).innerHTML =
                ...
                if (!element.email) {
                    document.querySelector(`.thong-tin-hanh-khach-email-${index}`).innerHTML = 'Vui lòng nhập email';
                    validateCount += 1;
                } else {
                    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contactInfomation.email)) {
                        document.querySelector(`.thong-tin-hanh-khach-email-${index}`).innerHTML = 'Email không hợp lệ';
                        validateCount += 1;
                    } else {
                        document.querySelector(`.thong-tin-hanh-khach-email-${index}`).innerHTML = '';
                    }
                }
            }

            if ((element.dob && isAbove14(element.dob)) && (!element.idType || element.idType == '')) {
                document.querySelector(`.thong-tin-hanh-khach-id-type-${index}`).innerHTML = 'Vui lòng chọn loại ID';
                validateCount += 1;
            } else {
                document.querySelector(`.thong-tin-hanh-khach-id-type-${index}`).innerHTML = '';
            }

            if ((element.dob && isAbove14(element.dob)) && !element.number) {
                document.querySelector(`.thong-tin-hanh-khach-passport-${index}`).innerHTML = 'Vui lòng nhập passport / cccd';
                validateCount += 1;
            } else {
                document.querySelector(`.thong-tin-hanh-khach-passport-${index}`).innerHTML = '';
            }
        });
        ...
        if (validateCount === 0) {
            ...
            router.push(`/dat-ve/them-dich-vu`);
        } else {
