'use client';

import { parseNgayThang, tinhThoiGianBay } from '@/lib/dateTime';

export default function FlightInfomation({ direction, flight }) {
    return (
        <div className="py-4 sm:py-6 text-start font-medium border-t">
            {/* Wrapper */}
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-y-4 sm:gap-y-0">
                {/* LEFT INFO */}
                <div className="col-span-1 text-sm sm:text-base">
                    <p className="text-sky-400 mb-1">
                        Chiều {direction}:
                        <p className='inline-block sm:hidden'>
                            <span className="text-gray-500 ml-2">
                                {flight.segments[0].flight.airlineCode.code}{' '}
                                {flight.segments[0].flight.flightNumber}
                            </span>
                        </p>
                    </p>
                    <p className='hidden sm:block'>
                        <span>Chuyến bay:</span>
                        <span className="text-gray-500 ml-2">
                            {flight.segments[0].flight.airlineCode.code}{' '}
                            {flight.segments[0].flight.flightNumber}
                        </span>
                    </p>
                    <p className="hidden sm:block">
                        <span>Airbus:</span>
                        <span className="text-gray-500 ml-2">
                            A{flight.segments[0].flight.aircraftModel.identifier}
                        </span>
                    </p>
                </div>

                {/* MAIN SEGMENT */}
                <div className="col-span-1 sm:col-span-3 grid grid-cols-1 sm:grid-cols-8 gap-y-3 sm:gap-y-0">
                    {/* Departure */}
                    <div className="col-span-1 sm:col-span-2">
                        <div className="font-semibold mb-1 text-sm sm:text-base">
                            <i>
                                {flight.segments[0].departure.airport.name}{' '}
                                ({flight.segments[0].departure.airport.code})
                            </i>
                        </div>
                        <div className="text-gray-500 text-xs sm:text-sm flex items-center">
                            {parseNgayThang(
                                flight.segments[0].departure.localScheduledTime,
                            ).time}
                            <span className="block h-2 w-2 rounded-full bg-gray-300 mx-2"></span>
                            {parseNgayThang(
                                flight.segments[0].departure.localScheduledTime,
                            ).date}
                        </div>
                    </div>

                    {/* Timeline */}
                    <div className="col-span-1 sm:col-span-4 px-0 sm:px-4">
                        <div className="text-sky-400 text-xs sm:text-sm w-full text-center mb-1">
                            {tinhThoiGianBay(
                                flight.segments[0].departure.localScheduledTime,
                                flight.segments[0].arrival.localScheduledTime,
                            )}
                        </div>

                        <div className="flex justify-between items-center relative">
                            <div className="w-2 h-2 bg-[url('/globalImages/Group16.png')] bg-cover"></div>
                            <div className="bg-[url('/globalImages/Vector1.png')] h-[1px] flex-grow"></div>
                            <div className="w-2 h-2 bg-[url('/globalImages/Group16.png')] bg-cover"></div>
                            <div className="w-5 h-5 bg-[url('/globalImages/airplane1.png')] bg-cover absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"></div>
                        </div>

                        <div className="text-gray-500 text-[10px] sm:text-xs w-full text-center mt-1">
                            Bay thẳng
                        </div>
                    </div>

                    {/* Arrival */}
                    <div className="col-span-1 sm:col-span-2 text-end">
                        <div className="font-semibold mb-1 text-sm sm:text-base">
                            <i>
                                {flight.segments[0].arrival.airport.name}{' '}
                                ({flight.segments[0].arrival.airport.code})
                            </i>
                        </div>
                        <div className="text-gray-500 text-xs sm:text-sm flex items-center justify-end">
                            {parseNgayThang(
                                flight.segments[0].arrival.localScheduledTime,
                            ).time}
                            <span className="block h-2 w-2 rounded-full bg-gray-300 mx-2"></span>
                            {parseNgayThang(
                                flight.segments[0].arrival.localScheduledTime,
                            ).date}
                        </div>
                    </div>
                </div>

                {flight.segments[1] && (
                    <>
                        <div className="hidden sm:block col-span-1"></div>
                        <div className="col-span-1 sm:col-span-3 grid grid-cols-1 sm:grid-cols-8 gap-y-3 sm:gap-y-0 mt-4 sm:mt-5">
                            {/* Departure */}
                            <div className="col-span-1 sm:col-span-2">
                                <div className="font-semibold mb-1 text-sm sm:text-base">
                                    <i>
                                        {flight.segments[1].departure.airport.name}{' '}
                                        ({flight.segments[1].departure.airport.code})
                                    </i>
                                </div>
                                <div className="text-gray-500 text-xs sm:text-sm flex items-center">
                                    {parseNgayThang(
                                        flight.segments[1].departure.localScheduledTime,
                                    ).time}
                                    <span className="block h-2 w-2 rounded-full bg-gray-300 mx-2"></span>
                                    {parseNgayThang(
                                        flight.segments[1].departure.localScheduledTime,
                                    ).date}
                                </div>
                            </div>

                            {/* Timeline */}
                            <div className="col-span-1 sm:col-span-4 px-0 sm:px-4">
                                <div className="text-sky-400 text-xs sm:text-sm w-full text-center mb-1">
                                    {tinhThoiGianBay(
                                        flight.segments[1].departure.localScheduledTime,
                                        flight.segments[1].arrival.localScheduledTime,
                                    )}
                                </div>

                                <div className="flex justify-between items-center relative">
                                    <div className="w-2 h-2 bg-[url('/globalImages/Group16.png')] bg-cover"></div>
                                    <div className="bg-[url('/globalImages/Vector1.png')] h-[1px] flex-grow"></div>
                                    <div className="w-2 h-2 bg-[url('/globalImages/Group16.png')] bg-cover"></div>
                                    <div className="w-5 h-5 bg-[url('/globalImages/airplane1.png')] bg-cover absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"></div>
                                </div>

                                <div className="text-gray-500 text-[10px] sm:text-xs w-full text-center mt-1">
                                    Bay thẳng
                                </div>
                            </div>

                            {/* Arrival */}
                            <div className="col-span-1 sm:col-span-2 text-start sm:text-end">
                                <div className="font-semibold mb-1 text-sm sm:text-base">
                                    <i>
                                        {flight.segments[1].arrival.airport.name}{' '}
                                        ({flight.segments[1].arrival.airport.code})
                                    </i>
                                </div>
                                <div className="text-gray-500 text-xs sm:text-sm flex items-center justify-end">
                                    {parseNgayThang(
                                        flight.segments[1].arrival.localScheduledTime,
                                    ).time}
                                    <span className="block h-2 w-2 rounded-full bg-gray-300 mx-2"></span>
                                    {parseNgayThang(
                                        flight.segments[1].arrival.localScheduledTime,
                                    ).date}
                                </div>
                            </div>
                        </div>
                    </>
                )}
            </div>
        </div>
    );
}
