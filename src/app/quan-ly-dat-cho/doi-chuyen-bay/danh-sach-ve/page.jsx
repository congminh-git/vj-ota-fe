...
import { getTravelOptionsUpdateJourney, getTravelOptionsAddJourney, formatTravelOptions } from '@/services/travelOptions/functions';
import {
...
export default function ListTravelOptions() {
    ...
    const paymentConfirm = async () => {
        if (updateJourneysStatus == 'update') {
            ...
            handlePutUpdateReservationJourneyByKey(reservationKey, journeyKey, body);
        } else {
            ...
            const response = await postReservationAddJourney(reservationKey, journeyAddWithPaymentTransaction);
    ...
    useEffect(() => {
        if(reservationKey && companyKey) {
            ...
            const getTravelOptionsAdd = async () => {
                const travelOptions = await getTravelOptionsAddJourney(departmentDate, cityPair, reservationKey, companyKey);
                setListFlightTravelOptions(formatTravelOptions(travelOptions))
            }
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-4 ">
                                <div className={`${roundTrip ? 'col-span-1' : 'col-span-2'}`}>
                                    <SelectedFlight
                                        ...
                                        arrivalAirport={`${arrivalCity?.replaceAll('_', ' ')} (${cityPair?.split('-')[1]})`}
                                            ...
                                            <div className="flex items-center py-2">
                                                ...
                                                >
                                                    ...
                                                    <span className="absolute text-blue-500 transition-opacity opacity-0 pointer-events-none top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 peer-checked:opacity-100">
                                                        ...
                                                        >
                                                            <circle
                                                                listFlightTraveloptions-name="ellipse"
                                                                cx="8"
                                                                cy="8"
                                                                r="8"
                                                            ></circle>
                                ...
                                {!roundTrip && departureFlight && quotations && paymentMethod.identifier ? (
                                    <button
                                        onClick={paymentConfirm}
                                        className="col-span-2 py-3 font-bold flex justify-center items-center rounded-md text-white bg-blue-500 border-2 border-blue-500 hover:bg-white hover:text-blue-500"
                                    >
                                ...
                                <div className={`${updateJourneysStatus == 'add' ? 'col-span-5' : 'col-span-7'}`}>
                                    ...
                                    {listFlightTraveloptions && listFlightTraveloptions.length > 0 ? (
                                        <ListFlightTravelOptions
                                            ...
                                            selectedFlight={activeSelectFlight == 'đi' ? departureFlight : returnFlight}
                                            setFareOptionFlight={
