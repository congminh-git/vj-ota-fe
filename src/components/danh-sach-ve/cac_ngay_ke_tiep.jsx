import ConsecutiveDayItem from './cac_ngay_ke_tiep_item';
...
function ConsecutiveDays({ flyingDay, activeSelectFlight, departmentDate, returnDate, roundTrip }) {
    ...
    const handleSearch = (date) => {
        ...
        const endPoint = pathname && pathname.includes('danh-sach-ve') ? 'danh-sach-ve' : 'dat-ve/danh-sach-ve';
        const url = `${endPoint}`;
        if (activeSelectFlight == 'đi') {
            sessionStorage.setItem(
                pathname.includes('dat-ve') ? 'departmentDateSearchParam' : 'departmentDateSearchParamUpdate',
                date,
            );
        } else {
            sessionStorage.setItem(
                pathname.includes('dat-ve') ? 'returnDateSearchParam' : 'returnDateSearchParamUpdate',
                date,
            );
        }
        if (roundTrip) {
