...
import { post9GSearch } from '@/services/search9G/functions';
...
export default function ListFlightPage() {
    ...
    const [list9GFlight, setList9GFlight] = useState();
        ...
        if (cityPair && companyKey) {
            ...
            const getListFlightTravelOptions = async () => {
                try {
                    ...
                    const result9G = await post9GSearch({
                        list_route: !returnFlight
                            ? [
                                  {
                                      departure: apiCallParams.cityPair.split('-')[0],
                                      arrival: apiCallParams.cityPair.split('-')[1],
                                      flight_date: apiCallParams.date,
                                  },
                              ]
                            : [
                                  {
                                  ...
                                  {
                                      departure: apiCallParams.cityPair.split('-')[1],
                                      arrival: apiCallParams.cityPair.split('-')[0],
                                      flight_date: returnDate,
                                  },
                              ],
                        adult: apiCallParams.adult,
                        child: apiCallParams.child,
                        infant: apiCallParams.infant,
                        option: {
                            direct_only: false,
                            flexible: false,
                            promo_code: '',
                            corporate_code: '',
                            trip_type: returnFlight ? "RT" : "OW",
                            currency: 'VND',
                        },
                    });

                    setListFlightTravelOptions(formatTravelOptions(result));
                    console.log(result9G.data)
                    setList9GFlight(result9G.data)
                } catch (error) {
                    router.replace('/');
                }
            };
    ...
    }, [apiCallParams, cityPair, companyKey, router, returnDate]);
                        ...
                        /> */}
                            <div>
                                <div className="grid grid-cols-2 sm:gap-4 gap-2 mt-4 ">
                                    <div
                                        id="select-flight"
                                        className={`${roundTrip ? 'col-span-2 md:col-span-1' : 'col-span-2'}`}
                                    >
                                        <SelectedFlight
                                            direction={'đi'}
